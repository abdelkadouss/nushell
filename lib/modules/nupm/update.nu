export def "nupm update" [] {
  ...
  for package in (ls --full-paths $data_path | where type == "dir" | get name) {
    let app_name = ($package | path basename);

    cd $package;
    if ( ls | where type == "dir" | where name =~ ".git" | is-not-empty ) {
      git pull;
    };
    if ( ls | where type == "file" | where name =~ "Cargo.toml" | is-not-empty ) {
      try {
        cargo build --release;

export def "nupm add" [repo: string, --remote_provider(-p): string = "github"] {
  let data_path = ( $env.NUPM_DATA_PATH? | default ($env.XDG_DATA_HOME | path join "nupm/packages") );
  mkdir $data_path;
  ...
  let endpoint = ($repo_url | path basename);
  ...
  let package_declaration_file_path = ( $env.config.plugins.nupm.NUPM_PACKAGE_DECLARATION_FILE_PATH? | default ( $env.NU_CONFIG_DIR | path join "packages.toml" ) );
  let bin_declaration_file_path = ( $env.config.plugins.nupm.NUPM_BIN_DECLARATION_FILE_PATH? | default ( $env.NU_CONFIG_DIR | path join "plugins.toml" ) );
  if not ( ( $package_declaration_file_path | path exists ) or ( $bin_declaration_file_path | path exists ) ) {
    error make {
      msg: $"The package declaration file '($package_declaration_file_path | path exists | default $bin_declaration_file_path)' does not exist."
      label: {
        text: "this package declaration file is not valid"
        span: (metadata ($package_declaration_file_path | path exists | default $bin_declaration_file_path)).span
      }
  ...
  if ( ls | where name =~ "nupm.nuon" | is-not-empty ) {
    if (open nupm.nuon | get -i type | is-not-empty) {
      $packgae_type = (open nupm.nuon | get -i type);
    };
  } else {
    if ( ls | where name =~ "Cargo.toml" | is-not-empty ) {
      $packgae_type = "custom";
    };
  };

  if ($env.config.plugins.nupm.NUPM_DIST_PATH? | is-empty) {
    print $"(ansi yellow_bold)warning(ansi reset). you have not set the 'NUPM_DIST_PATH' config fild. the packages will be installed to the default path '($env.NUPM_HOME | path join 'plugins')' make sure to add this path to $env.NU_LIB_DIRS list to easy use or set the var and reinstall the plugin"
  };

  match $packgae_type {
    "script" => {script_add $package_name}
    "custom" => {bin_add $package_name $bin_declaration_file_path $package_declaration_file_path $repo_url}
    "module" => {module_add}
    _ => { error make {
...
def script_add [package_name: string] {
  ...
  if ( ls | where type == "file" | get -i $file | is-not-empty ) {
    print $"(ansi green_bold)will i think i find the correct file!. '($package_name)'(ansi reset)"
    let dist_path = ( $env.config.plugins.nupm.NUPM_DIST_PATH? | default ($env.NUPM_HOME | path join "plugins") );
    let dist_script_path = ($dist_path | path join "scripts");
    ...
    print $"(ansi red_bold)failed(ansi reset). u have to add the file symlinks manually.\n (ansi green) help:\n$ cd \($env.NUPM_DATA_PATH? | default \($env.XDG_DATA_HOME | path join 'nupm/packages'))\n$ cd ($package_name)\n$ ls"
  };
}

def bin_add [package_name: string, bin_declaration_file_path: string, package_declaration_file_path: string, repo_url: string] {
  let dist_path = ( $env.config.plugins.nupm.NUPM_DIST_PATH? | default ($env.NUPM_HOME | path join "plugins") );
  let dist_bin_path = ($dist_path | path join "bin");
  mkdir $dist_bin_path | ignore;

  cargo build --release;
  if (($dist_bin_path | path join $package_name) | path exists) {
